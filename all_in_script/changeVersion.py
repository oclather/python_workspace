...
def changeVersion(filename,version):
    ...
    print "Write "+color.colorText.red(filename)+color.colorText.green(" success!\n")
    print "*"*80

filepath = raw_input(r"please input filepath:").strip()#用raw_input  才不会出现windows路径报错的问题；strip() 用来去掉结为的/r
print "*"*80
