...
def main():
    ...
    orgin_path = raw_input(encodeString(r"请输入要复制资源的原始路径，以版本号结尾："))
    targetpath = raw_input(encodeString(r"请输入要复制资源的目标路径，以版本号结尾："))
    #targetpath = r"E:\cdn_korea\android"
    #version = reString(r"v\d{1,4}.\d{1,4}.\d{1,4}.\d{1,4}",orgin_path)[0] #原来的正则：(?<=android\\).*?(?=_)|(?<=ios\\).*?(?=_)

    hotinfo_old = targetpath + "\\hot\\res\\info.txt"
    #print hotinfo_old
    hotinfo_new = orgin_path + "\\hot\\res\\info.txt"
    #print hotinfo_new
    opinfo_old = targetpath + "\\optional\\res\\opinfo.txt"
    #print opinfo_old
    opinfo_new = orgin_path + "\\optional\\res\\opinfo.txt"
    #print opinfo_new
    copypath1 = orgin_path + "\\hot\\res\\"
    copypath2 = orgin_path + "\\optional\\res\\"
    dstPath1 = targetpath + "\\hot\\res\\"
    #print dstPath1
    dstPath2 = targetpath + "\\optional\\res\\"
    #print dstPath2
    ...
    if os.path.exists(hotinfo_old):#判断是否已经有旧资源。
        ...
        print encodeString("记得检查和修改info中的版本号！！！")
        print encodeString("记得检查和修改info中的版本号！！！")
        print encodeString("记得检查和修改info中的版本号！！！")
        '''
        ...
        print encodeString("目标位置没有该版本信息，请检查输入目录")
